...
private val patchOneHotCodes = listOf(
    "2X1_I",
    "2X2_L",
    "2X2_L2",
    "2X2_O",
    ...
    "5X1_I",
    "5X3_X",
).mapIndexed { idx, patchId ->
    patchId to BigInteger.ONE
        .shiftLeft(idx)
        .toString(2)
        .padStart(33, '0')
        .map {
            when (it) {
                '0' -> 0f
                '1' -> 1f
                else -> throw CharConversionException()
            }
        }
}.toMap()

fun String.asPatchIdOneHotEncodedFloatList() =
    (patchOneHotCodes[this] ?: throw ArrayIndexOutOfBoundsException())
